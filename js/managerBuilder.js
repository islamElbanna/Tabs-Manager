    ...
    chrome.runtime.sendMessage({cmd: CMD_GET_TABS_DETAILS, tabsIds: tabsIds}, function (tabsDetails) {
        ...
        if (keyword) {
            for (let i in tabsDetails) {
                ...
                if (tab[TABS_DETAILS_TITLE].toLowerCase().indexOf(keyword.toLowerCase()) !== -1) {
                    filteredTabs.push(tab);
                }

            }
            tabsDetails = filteredTabs;
        }


        console.log("raw tabs : ", tabsDetails)
...
function buildGroupTabs(groupTabsDetails, windowIdMpping, windowsCount, isOthersGroup) {
    let groupSection = "";
    let tabsArray = Object.values(groupTabsDetails).sort((a,b)=>b[TABS_DETAILS_ACTIVE_TIME]- a[TABS_DETAILS_ACTIVE_TIME]);
    for (let i in tabsArray) {
        let tabId = tabsArray[i][TABS_DETAILS_TAB_ID];
        let tabDetails = tabsArray[i];
        if (tabDetails[TABS_DETAILS_TITLE]) {
            ...
            groupSection += '<a class="thumbnailImg tab" href="#" >' +
                '<img style="max-height: 150px" src="' + img + '" />' +
                '</a>' +
                '</div>' +
                '<div class="card-footer">' +
                '<div class="title-section">';
            if (isOthersGroup)
                groupSection += '<img  style="max-height: 150px" src="' + icon + '" class="groupIcon">';
...
function groupTabs(tabsdDetails) {
    ...
    for(let i in arr.slice(0,6)){
        let tab = arr[i];
        pinnedGroup[tab[TABS_DETAILS_TAB_ID]] = tab;
    }
...
function addEvents() {
    ...
    $('.item').hover(function () {
        ...
        $(this).find('.thumbnail img').stop(false, true).animate({
            'width': width,
            'height': height,
            'top': move,
            'left': move
